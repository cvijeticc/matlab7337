...
## Na nekoliko mesta u skripti su dodate napomene kako bi se ukazalo na
 neka malo bolja resenja
...
//ako vrati true znaci da su sve vrednosti true, a da je vratilo 
false znacilo bi da postoji bar jedna vrednost koja je false
...
cv <- train(
  Stayed ~ .,                   # “y ~ X”: y je Stayed; X su SVE OSTALO (tačka = sve ostale kolone)
  data = train_data,            # odakle da uzme kolone
  method = "rpart",
  trControl = tr_ctrl,
  tuneGrid = cp_grid
)
//ovo je mozda bolja prica
